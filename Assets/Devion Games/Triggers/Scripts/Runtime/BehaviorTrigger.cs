...
namespace DevionGames
{
    ...
    public class BehaviorTrigger : BaseTrigger
    {
        ...
        [SerializeField]
        protected bool m_isBehaviorTrigOnly = false;

        //Task behavior that runs custom actions
        protected Sequence m_ActionBehavior;
        ...
        {
            // Custom code for HeroesTale, disable range check when it's 0
            if (useDistance != 0)
            {
                if (!InRange) { return; }
            }
        ...
        public override bool Use()
        {
            ...
            // if trigger range == 0 then ignore range check
            if (useDistance != 0)
            {
                if (!CanUse())
                {
