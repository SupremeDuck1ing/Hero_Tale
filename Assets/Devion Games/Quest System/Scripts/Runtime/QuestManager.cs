...
namespace DevionGames.QuestSystem
{
    public class QuestManager : MonoBehaviour
    {
        ...
        private static void LoadQuests(string json, ref List<Quest> quests, bool registerCallbacks = false)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (quest != null)
                {
                    ...
                    quests.Add(instance);
                    current.AddQuest(instance);
                    current.AllQuests.Add(instance);
                    //Debug.Break();
                }
                else {
        ...
        public Quest GetAnyQuest(string name) {
            return this.AllQuests.FirstOrDefault(x => x.Name == name);
        }

        public bool HasQuest(string nameOfQuest, Status status) {
            List<Quest> quests = QuestManager.current.AllQuests;
            foreach (Quest aQuest in quests)
            {
                if (aQuest.Title == nameOfQuest)
                {
                    if (aQuest.Status == status)
                    {
                        return true;
                    }
                    else
